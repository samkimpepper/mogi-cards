...
**fix2 완료 (2026-08-14, 커밋 336a3d1)** — 정리 실패가 원작자 삭제를 못 막게(서브트랜잭션+EXCEPTION), 실패분은 `storage_cleanup_pending` 기록(클라이언트 완전 비노출, sw-u4r가 소비 인수), 임시 조치 선언(보장=공개 서빙 차단 / 비보장=물리 바이트 / 대상 밖=hidden — 공식 문서 원문 확인 인용). 실패 경로 테스트(해제 스위치 소멸 모사 → 삭제 성공+pending 1행) 통과. 마스터 최종 재실측(02:52Z): CI 7건 결론·MERGEABLE·늦은 리뷰 0·스냅샷 코멘트 pending 테이블 반영 갱신 확인. vitest 1건 실패는 flaky 실측(단독 40/40). **머지만 남음.**
