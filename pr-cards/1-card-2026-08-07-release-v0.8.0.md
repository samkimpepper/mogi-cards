...
## 2026-08-10 갱신 (냐옹이 Gen 2) — 최신 dev 편입 후 재검증 `[READ]`

- `[READ]` 모기 결정으로 release 브랜치에 **컷 이후 dev 13커밋을 추가 편입**했다 (#478~#491: 매직링크 429 카피 분리, users 유령 정책 제거, 등록 funnel 계측, 알림 src 계측 등). 이제 main..release = **147커밋**, 충돌 0건. 1~4절의 구조 설명은 그대로 유효하다.
- `[EVIDENCE]` 갱신 트리 재검증 (워커 + 마스터 이중): gitleaks 612 commits no leaks · pnpm build PASS(967ms) · wiki:lint 경고 4건(컷 시점과 동일한 기존 건) · verify-agent-ssot PASS · PR checks 6/6 non-empty · unresolved 스레드 0 · MERGEABLE/CLEAN.
- `[READ]` **vitest 주의 하나**: 전체 스위트는 546/548 — 실패 2건은 Atlas 밀집 fixture 테스트의 5초 타임아웃(부하 상태에서 8.3s/5.9s 소요)이고, 같은 파일 단독 실행은 40/40 통과. 테스트 파일은 컷 이전부터 불변이라 **이번 편입의 회귀가 아니다**. 타임아웃 여유 조정은 별도 이슈로 분리(sw 트래커).
- `[DECIDE]` R1~R3 퀴즈는 이미 통과했고 갱신분은 구조 변화가 없으므로, 이 절만 읽으면 머지 판단 가능.
