...
- 모기가 현재 문서를 충분히 봤거나 자기 판단으로 넘어가겠다고 말하면 과외 세션이 `reviewed: true`로 바꾸고 커밋한다.
- `reviewed`는 읽기 종료 표시다. PR 머지 준비 판단은 별도 `merge_ready`로 기록하며, 모기가 명시적으로 머지 가능하다고 말했을 때만 `true`가 된다.
- 상태 갱신은 mogi-cards에 허용된 쓰기다. 제품·운영 저장소에는 영향을 주지 않는다.
