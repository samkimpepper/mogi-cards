...
- 상태 값을 `true`로 바꾸는 일은 모기가 파일에서 직접 하거나 아래 로컬 상태판의 체크박스를 직접 눌러 실행한다. 과외 세션은 종료 표현·퀴즈 통과·머지 가능 발언을 듣고도 자동 변경하거나 대신 클릭하지 않는다.
- 과외 세션은 현재 상태의 의미를 설명하고 새 문서를 만들 때 `false` 기본값만 기록한다.

### 읽은 문서 상태판

Orca의 Markdown 체크박스 대신 로컬 상태판에서 읽은 문서를 한 번 클릭해 표시할 수 있다. 과외 세션의 `SessionStart` 훅이 과외냥이에게 자동 시작을 지시하고, 과외냥이가 상태판을 시작하거나 이미 실행 중인 서버를 재사용하므로 모기가 실행 명령을 외울 필요는 없다. 과외냥이는 시작 인사에 `http://127.0.0.1:8766` 링크를 함께 보여준다.

자동 시작이 고장 났을 때의 수동 복구 명령은 다음과 같다. 평소에는 모기가 직접 실행하지 않고 과외냥이가 복구한다.

```bash
node review-dashboard/server.mjs
```

실행 뒤 Orca 브라우저에서 `http://127.0.0.1:8766`을 연다. 상태판은 `pr-cards/`·`plan-cards/`·`code-diff-notes/` 바로 아래의 `reviewed: false` 문서만 보여주며, 모기가 체크한 문서의 frontmatter만 `reviewed: true`로 바꾼다. `temp/`·`archive/`·상태 필드가 없는 레거시는 제외하고 commit과 push는 자동으로 하지 않는다. 종료할 때는 상태판을 실행한 터미널에서 `Ctrl+C`를 누른다.

## 기록된 선택 다시 판단하기

- 모기가 PR 본문을 열기 전에 과거 AskUserQuestion 판단과 현재 판단을 비교하려 하면, 과외 세션이 PR·diff·연결된 설계 원문을 먼저 읽고 `추천 비공개 결정 브리프`를 채팅으로 전달한다.
- 브리프에는 필요한 사실·제약·불확실성과 가능한 대안을 넣고, 예상 사용자 동작·구현 비용·사용자 손해·되돌리기 가능성·미확인 사항을 같은 기준으로 보여준다.
- 새 답을 받기 전에는 과거 선택지의 표현·에이전트 추천·모기의 당시 선택·실제 구현·구현 뒤 결과를 공개하지 않는다.
- 답을 받은 뒤 `당시 판단 / 예상 비용만 본 현재 판단 / 구현 결과까지 본 사후 판단`을 구분해 비교한다. 다르더라도 오답이나 능력 점수로 취급하지 않는다.
- 모기가 이미 PR 본문이나 구현 결과를 읽었다면 독립 판단으로 재연하지 않고 회고 비교만 한다.
