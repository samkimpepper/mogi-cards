...
**따라서 부담은 쓰는 쪽으로 넘어온다 (마스터·워커 의무)**

완독으로 잡히려면 중요한 것이 **눈에 걸리는 자리에** 있어야 한다. PR 본문·카드를 쓸 때:

1. **묻지 마라** — 되돌릴 수 없는 것, 데이터가 지워지는 것, 수용한 위험은 문단 한가운데가 아니라 **절 제목이나 첫 문장**에 둔다.
2. **긴 산문 대신 표·목록** — 완독은 구조를 타고 흐른다. 근거가 길면 본문엔 결론 한 줄, 상세는 링크로.
3. **"이 문서에 없는 것"을 적는다** — 원문에서 직접 봐야 할 대목 1~2곳을 지목한다(포스트모템 069 개선안 I8과 짝).
4. 모기가 완독하고도 놓쳤다면 **본문 배치의 문제로 본다.** 다시 읽으라고 하지 않는다.
