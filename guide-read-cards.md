...
audit_log:
  ...
  - action: updated
    at: 2026-08-20
    by: Codex
    note: "잘못 분리된 lead-cards를 pr-cards로 통합하고, 요청형 한 줄 체크 워크북을 code-diff-notes/temp 예외로 분리."
---
...
mogi-cards에 배달한 개인 공책 사본은 읽는 목적에 따라 `pr-cards/`·`plan-cards/`·`code-diff-notes/`에 둔다. 실제 PR 승인 카드는 `pr-cards/`, 구현 전 설계·SPEC은 `plan-cards/`, 실제 코드·diff 학습은 `code-diff-notes/`다. 파일명은 `<날짜>-<주제>.md`로 시작하고 `0-`·`1-card-`·`2-note-` 같은 숫자·종류 prefix를 붙이지 않는다. 문서 종류는 폴더가 이미 표현한다.

`code-diff-notes/temp/`는 모기가 피곤한 세션에 한 줄씩 체크하며 읽고 싶다고 명시적으로 요청했을 때만 만드는 임시 워크북 보관처다. 상시 산출물이나 독립 활성 큐가 아니며 세션 부트에서 자동 제시하지 않는다.
...
(코드 학습)이고, 지난 자료는 `archive/`다. `code-diff-notes/temp/`는 요청형 한 줄 체크
워크북 예외이며 활성 큐가 아니다. 세션 부트는 세 활성 폴더 바로 아래 문서 중 최신 1개만 보여준다.
...
| 등급 | 대상 | 게이트 |
|---|---|---|
| 본문 완독 | docs · 테스트만 · 카피/문구 · 동작 불변 리팩토링 | PR 본문을 **처음부터 끝까지** 읽고 머지. 퀴즈 없음 — 단 "안 읽고 머지"도 없음 |
...
### 완독 ≠ 정독 (모기 정정 2026-08-19)

모기가 등급 이름을 정정했다: *"정독은 못 해(모기 힘들어). 완독은 할 수 있다."* 등급 ①의 이름을 **본문 정독 → 본문 완독**으로 고친 근거다. 지킬 수 없는 이름을 걸어두면 그 규칙은 조용히 안 지켜진다.

- **완독** = 본문을 처음부터 끝까지 지나간다. 모든 문장을 곱씹지는 않는다. **모기가 약속한 것.**
- **정독** = 문장마다 의미를 따져가며 읽는다. 등급 ③의 퀴즈가 그걸 대신한다 — 사람에게 정독을 요구하는 대신 질문으로 확인한다.

**완독의 목적은 이해가 아니라 접촉이다 (모기 정정 2026-08-19, 2차)**

모기: *"PR 본문을 또 모기 맞춤형으로 만들고 어쩌구 하는 데 비용을 쓰고 싶지 않다. 그건 리드 카드가 하는 거고, 난 그냥 PR 본문을 끝까지 '읽기만' 하는 거다 — 이해가 되든 안 되든. **접촉**을 하는 거다."*

역할이 갈린다. 섞지 마라:

| 산출물 | 독자 | 목적 | 누가 맞춘다 |
|---|---|---|---|
| **PR 본문** | 저장소·미래의 개발자 | 변경의 정본 기록 | 저장소 규약. **소유자 맞춤 금지** |
| **리드 카드** | 모기 | 이해를 만든다 | 마스터가 얇게·모기 맞춤으로 |

따라서:

- **PR 본문을 모기용으로 다시 쓰지 않는다.** 앞 판(2026-08-19 1차)에 적었던 "묻지 마라·표로 바꿔라·이 문서에 없는 것을 적어라"는 **본문 의무에서 철회한다** — 그건 카드가 이미 하는 일이고, 본문에 얹으면 같은 가공을 두 번 하는 비용이다. 본문은 저장소 관례(Problem / Why this approach / What this changes / Expected effect)를 그대로 지킨다.
- **완독은 이해를 목표로 하지 않는다.** 끝까지 지나가는 것 자체가 목적이다 — 가공본에만 노출된 상태를 깨는 것. 이해는 카드와 퀴즈가 맡는다.
- **그래서 완독은 이해 없이도 값을 낸다.** 접촉하면 걸리는 것이 생기고, 걸리면 질문이 나온다. 실증은 아래 #521 — 모기는 본문의 glob 규칙을 "이해"한 게 아니라 **눈에 걸려서 물었고**, 그 질문 하나가 감시선 구멍 둘을 열었다.
- 모기가 완독하고도 못 본 것을 두고 **다시 읽으라고 하지 않는다.** 그건 카드가 못 나른 것이다.
